...
class XTEvernoteTransition: NSObject, UIViewControllerAnimatedTransitioning, UIViewControllerTransitioningDelegate, NoteViewControllerDelegate {

    internal var isPresent = true
    var selectCell:XTCollectionViewCell = XTCollectionViewCell()
    var visibleCells = [XTCollectionViewCell]()
    ...
    func EvernoteTransitionWith(selectCell selectCell:XTCollectionViewCell, visibleCells:[XTCollectionViewCell], originFrame:CGRect, finalFrame:CGRect, panViewController:UIViewController, listViewController:UIViewController) {
        ...
        let pan = UIScreenEdgePanGestureRecognizer(target: self, action: #selector(XTEvernoteTransition.handlePanGesture(_:)))
    ...
    func animateTransition(transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animateWithDuration(self.transitionDuration(transitionContext), delay: 0.0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { () -> Void in
            for visibleCell:XTCollectionViewCell in self.visibleCells {
